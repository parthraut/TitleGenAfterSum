# 📝 Title Generation After Summarization

Exploring the connection between summarization and title generation 🚀.

### 🤔 Introduction
Does summarization improve title generation? Our project delves into this question, leveraging models like BART and GPT-2.

### 🧭 Navigating the Repository

- **🔍 Training Code:** Discover our model training code at `src/finetuning/`.
- **📊 Evaluation Code & Results:** Find our evaluation methods and results in `src/evaluation/`.
- **👥 Human Evaluation:** For insights from our human evaluation process, visit `human_eval/`.

This repository is your guide to understanding our project and the intersection of summarization tasks and effective title generation models.

**Note:** Because of the large size of our GPT-2 models and their poor performance on title generation, we decided not to include these models in this repository. However, they can me made available upon request. 
